# Customer: Shipyard Brewing Company
...
```bash
sf org list | grep shipyard
```

Connect to an org:
```bash
bash scripts/connect-org.sh shipyard --sandbox --type sandbox
```
...
- Shared integration code (goes in `integrations/vip-srs/`)
...
- Read `integrations/vip-srs/` for VIP SRS integration source code and documentation
