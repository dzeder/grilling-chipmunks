...
```bash
sf org list | grep beverage-market
```

Connect to an org:
```bash
bash scripts/connect-org.sh beverage-market --production --type customer
bash scripts/connect-org.sh beverage-market --sandbox --type sandbox
```
