...
def test_registry_contains_every_sf_skill_directory() -> None:
    ...
    # Skills are now organized under skills/salesforce/ pillar
    skill_dirs = sorted(path.name for path in (ROOT / "skills" / "salesforce").glob("sf-*") if path.is_dir())
...
def test_registry_does_not_reference_missing_sf_skill_directories() -> None:
    registry_skills = load_registry()["skills"]
    # Skills are now organized under skills/salesforce/ pillar
    skill_dirs = {path.name for path in (ROOT / "skills" / "salesforce").glob("sf-*") if path.is_dir()}
