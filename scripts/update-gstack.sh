...
if [ "$MODE" = "--apply" ]; then
  ...
  echo "  2. Rebuild browser: cd .claude/skills/gstack && ./setup && cd -"
  echo "  3. Re-normalize symlinks (setup creates absolute paths): bash scripts/fix-gstack-symlinks.sh"
  echo "  4. Commit: git add .claude/skills/ && git commit -m 'chore: update gstack to ${UPSTREAM_VERSION}'"
  echo ""
  echo "⚠️  IMPORTANT: Always run step 3 after step 2. The setup script creates"
  echo "   absolute symlinks that break when the repo moves. CI will block merges"
  echo "   with broken symlinks."
else
